...
Main.recite:
 ...
 # formal value
	...
	jr	$ra	 # recite
Main.disregard:
	addiu	$sp $sp -12 # disregard
	sw	$fp 12($sp) # disregard
	sw	$s0 8($sp) # disregard
	sw	$ra 4($sp) # disregard
	addiu	$fp $sp 4 # disregard
	move	$s0 $a0 # disregard
 # formal a
 # formal b
 # formal c
# for self SELF 
	move	$a0 $s0 # disregard
	lw	$fp 12($sp) # disregard
	lw	$s0 8($sp) # disregard
	lw	$ra 4($sp) # disregard
	addiu	$sp $sp 24 # disregard
	jr	$ra	 # disregard
Main.main:
	addiu	$sp $sp -12 # main
	sw	$fp 12($sp) # main
